...
document.getElementById("city").addEventListener("change", async function () {
    ...
    if (selectedCity) {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5050/api/localities?city=${selectedCity}`);
            ...
            localitiesGroup.querySelectorAll(".checkbox-item:not(.select-all-item)").forEach(el => el.remove());
...
document.getElementById('bookingForm').addEventListener('submit', function (e) {
    ...
    // Get selected brands
    const selectedLocalities = Array.from(document.querySelectorAll('input[name="localities"]:checked'))
